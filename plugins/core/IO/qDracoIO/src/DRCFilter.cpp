...
CC_FILE_ERROR DRCFilter::saveToFile(ccHObject* entity, const QString& filename, const SaveParameters& parameters)
{
	...
	else if (entity->isKindOf(CC_TYPES::MESH))
	{
		...
		if (!draco::Encoder().EncodeMeshToBuffer(dracoMesh, &buffer).ok())
		{
	...
	else if (entity->isKindOf(CC_TYPES::POINT_CLOUD))
	{
		...
		if (!draco::Encoder().EncodePointCloudToBuffer(dracoCloud, &buffer).ok())
		{
			return CC_FERR_THIRD_PARTY_LIB_FAILURE;
		}
	}
...
static CC_FILE_ERROR LoadCloud(ccPointCloud& ccCloud, const draco::PointCloud& dracoCloud, FileIOFilter::LoadParameters& parameters)
{
	...
	if (nullptr != normalAttribute && normalAttribute->data_type() == draco::DataType::DT_FLOAT32)
	{
		if (ccCloud.reserveTheNormsTable())
		{
			assert(pointCount == normalAttribute->size());
	...
	if (nullptr != colorAttribute && colorAttribute->data_type() == draco::DataType::DT_UINT8)
	{
		...
		if (rgb || rgba)
		{
			...
			{
				assert(pointCount == colorAttribute->size());
				if (rgb)
				{
	...
	if (nullptr != sfAttribute && sfAttribute->data_type() == draco::DataType::DT_FLOAT32)
	{
		ccScalarField* sf = new ccScalarField();
		if (sf->reserveSafe(pointCount))
		{
			assert(pointCount == sfAttribute->size());
...
CC_FILE_ERROR DRCFilter::loadFile(const QString& filename, ccHObject& container, LoadParameters& parameters)
{
	...
	if (geomType == draco::TRIANGULAR_MESH)
	{
		...
		for (draco::FaceIndex f(0); f < meshDraco->num_faces(); ++f)
		{
			...
			assert(face.size() == 3);
			uint32_t i1 = face[0].value();
			uint32_t i2 = face[1].value();
			uint32_t i3 = face[2].value();
